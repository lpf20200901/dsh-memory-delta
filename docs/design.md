...
- **M5 ✅（2026-09-17）**：把"记忆真的有用"这件事做扎实 —— 三块**主体能力**，
  ...
  - 测试 **695 个断言全绿**（CLI 147 + planner 43 + search 51 + due 93 + hook 63 + plugin 181 + client 117），
    真机预检 15/15。
...
1. **面板分组头**补上流程位置与注入与否：`事实（facts）已确认的世界结论 · 参与注入`、
   `决策（decisions）已确认的约定 · 参与注入`、`收件箱候选（inbox）待确认 · 不注入`；
   状态行在有归档时补一个 `归档（archive）N 条` —— 归档层本来不出现在面板里，
   但"它存在"这件事得让用户看见，否则被取代的东西像是凭空消失了。
...
而"只改说法"已经把"分不清"解决掉了 —— 结构留给真有需求时再动。
