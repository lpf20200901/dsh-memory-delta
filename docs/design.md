...
- **M5 ✅（2026-09-17）**：把"记忆真的有用"这件事做扎实 —— 三块**主体能力**，
  ...
  - 测试 **825 个断言全绿**（CLI 180 + planner 43 + search 51 + due 93 + hook 63 + plugin 217 + client 178），
    真机预检 15/15。
...
**第三稿（用户再次反馈后）：把 `事实`/`决策` 的描述换成人话。**
用户的原话很准：**"事实"说白了就是完成任务过程中遇到的坑及避免的方法，"决策"直白点就是我的决定
（业务等上面的约定），防止你再问我或犯糊涂的记录**。于是面板里：

- 分组头：`事实（facts）踩过的坑 & 绕开的方法` / `决策（decisions）你定下的约定`
- 每组正文第一行给"写起来的诀窍"：
  `换个环境或换个版本，它可能就不成立了 —— 所以写清"什么情况下适用"最有价值。` /
  `业务/流程/口味上的决定：只有你改主意才会变 —— 记下"为什么这么定"，我就不会再问第二遍。`
- 库自带说明书与项目 README 的对照表同步改写

**判据保留**（"明天世界变了，这条会不会失效？"）：人话负责"一眼看懂记什么"，
判据负责"拿不准时该放哪边" —— 两者不能互相替代，所以两处都留着。
注入正文里的分节标题仍是 `### 事实 / ### 决策`，**没改成俏皮话**：
那部分是每轮都要花的字节，且模型需要的正是"事实 vs 决策"这个短标签，不是解释。
...
**⑩ 三个维度：类型 / 标签 / 日期（M11）。**
用户问能不能"按年月日区分"。能，而且**纯客户端**（条目本来就带 `date`，宿主不用改、不用重启）：

- 头部一个三档切换：**类型**（事实/决策 —— 回答"该放哪边"）/ **标签**（取第一个标签，按主题）/
  **日期**（同一天一组，新的在前）。
- 日期分组带**人话标注**：今天 / 昨天 / 前天 / 周几 —— 比一串 ISO 日期好读
  （用宿主回传的 `state.today` 算差，不靠浏览器本地时区猜）。
- 日期视图里条目**自己标出**是事实还是决策（复用已有的 `showType`），否则就丢了类型信息。
- 没写 `date` 的条目归到最后的「没有日期」，不静默丢掉。

**为什么不做"按年月"或时间轴**：日粒度已经能回答"我什么时候记的这条"；再往上就是
时间轴/热力图那一类"看板"，早前已判定为过度设计（用户明确否掉过仪表盘与计数器）。
